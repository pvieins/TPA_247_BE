...
namespace PVI.API.Web247.Controllers
{
    ...
    public class DmUyQuyenController : ControllerBase
    {
        ...
        [HttpGet]
        public IActionResult getListUyQuyen()
        {
            try
            {
                var types_uyquyen =  JsonConvert.SerializeObject(_uqService.GetTypeUyQuyen());
                return Ok(types_uyquyen);
            } catch (Exception ex)
            {
        ...
        public IActionResult createUyQuyen([FromBody] UyQuyenRequest entity)
        {
            try
            {
                ...
                {
                    if (entity.GhSotienUq < 0)
                    {
                        return BadRequest("Số tiền ủy quyền không được nhập số âm");
                    }
                }
                string currentUserEmail = JwtTokenHelper.ExtractTokenInfoAndSetEmail(HttpContext);
