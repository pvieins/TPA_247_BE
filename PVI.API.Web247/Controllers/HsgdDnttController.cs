...
namespace PVI.API.Web247.Controllers
{
    ...
    public class HsgdDnttController : ControllerBase
    {
        ...
        [HttpGet("GetThongtinTKThuHuong")]
        [Authorize]
        public async Task<IActionResult> GetThongtinTKThuHuong(decimal pr_key_hsgd)
        {
            try
            {
                var entities = await _hsgdDnttService.GetThongtinTKThuHuong(pr_key_hsgd);
                _logger.Information("GetListNhomKT success");
                return Ok(entities);
            }
            catch (Exception ex)
