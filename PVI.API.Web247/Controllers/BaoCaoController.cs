    ...
    public class BaoCaoController : ControllerBase
    {
        ...
        public IActionResult ThongKeGDTT([FromBody] ThongKeGDTT_General_Main_Filter filter, int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                ...
                ThongKeGDTT_General_Response list_gia_pt = _BaoCaoService.ThongKeGDTT(filter, pageNumber, pageSize, currentUserEmail).Result;
                //_logger.Information("GET request TraCuuGiaPhuTung received");
                if (list_gia_pt != null)
                {
                    return Ok(list_gia_pt);
                ...
                {
                    return BadRequest("Báo cáo 03 chạy lỗi, vui lòng check log");
                }
            }
            catch (Exception ex)
            {
                _logger.Error($"An error occured when running BC03: {ex.Message}");
        ...
        public IActionResult BaoCaoThuHoiTS([FromBody] BCThuHoiTS_Main_Filter filter, int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                string currentUserEmail = JwtTokenHelper.ExtractTokenInfoAndSetEmail(HttpContext);
                //string currentUserEmail = "longnh4@pvi.com.vn";
                 BCThuHoiTSItemResponse result = _BaoCaoService.BCThuHoiTS(filter, pageNumber, pageSize, currentUserEmail);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        // Lấy tất cả danh sách device theo filter.
        [HttpPost("ExportBCThuHoiTS")]
        [Authorize]
        public IActionResult ExportBaoCaoThuHoiTS([FromBody] BCThuHoiTS_Main_Filter filter)
        {
            try
            {
                string currentUserEmail = JwtTokenHelper.ExtractTokenInfoAndSetEmail(HttpContext);
                //string currentUserEmail = "longnh4@pvi.com.vn";
                BCThuHoiTSItemResponse result = _BaoCaoService.ExportBCThuHoiTS(filter, currentUserEmail);
                return Ok(result);
            }
            catch (Exception ex)
            {
