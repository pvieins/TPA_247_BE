...
namespace PVI.API.Web247.Controllers
{
    ...
    public class DmUserController : ControllerBase
    {
        ...
        public async Task<IActionResult> searchUserGDTT([FromBody] UserGDTT searchTarget, int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                var currentUserEmail = JwtTokenHelper.ExtractTokenInfoAndSetEmail(HttpContext);
                //string currentUserEmail = "longnh4@pvi.com.vn";
                var list_user = await _dmUserService.searchUserGDTT(pageNumber, pageSize, searchTarget, currentUserEmail);
                if (list_user == null)
                {
        ...
        [Authorize]
        // Có thể thêm filter vào để xuất thông tin theo trường.
        public async Task<IActionResult> searchUserGDDK([FromBody] UserGDDK searchTarget, int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                var currentUserEmail = JwtTokenHelper.ExtractTokenInfoAndSetEmail(HttpContext);
                //string currentUserEmail = "longnh6@pvi.com.vn";
                var list_user = await _dmUserService.searchUserGDDK(pageNumber, pageSize, searchTarget, currentUserEmail);
                if (list_user == null)
                {
                    _logger.Error("Lỗi khi search user.");
                    return NotFound();
        ...
        public async Task<IActionResult> createUserGDTT ([FromBody] UserGDTT entity)
        {
            try
            {
                ...
                if (!result.Contains("Mã")) // Nếu không báo lỗi
                {
                    return Ok(result);
                }
                else
        ...
        [Authorize]
        public IActionResult UpdateUserGDDK([FromBody] UserGDDK entity)
        {
            try
            {
                if (entity == null)
                ...
                var currentUserEmail = JwtTokenHelper.ExtractTokenInfoAndSetEmail(HttpContext);
                var result = _dmUserService.updateUserGDDK(entity, currentUserEmail).Result;
                _logger.Information("PUT UpdateUserGDDK request up received");
                if (!result.Contains("Mã")) // Nếu không báo lỗi
                {
                    return Ok(result);
                }
